...
BASE_DIR = '/mnt/c/Users/thiago.silva/Downloads/portfolio'
...
if __name__ == '__main__':
    ...
    datas = [(datetime.today() - timedelta(days=i)).strftime('%Y-%m-%d') for i in range(5)]
    
    for DATA_PROCESSAMENTO in datas:
        print(f'\n>>> Processando data: {DATA_PROCESSAMENTO}')
        ingest_bronze_to_silver(sistema='sistema_devolucao', dominio='devolucao', nome_tabela='devolucao', prefixo='devolucao', data_processamento=DATA_PROCESSAMENTO, coluna_date='data_devolucao', coluna_id='pedido_id')
        ingest_bronze_to_silver(sistema='sistema_estoque', dominio='estoque', nome_tabela='estoque', prefixo='estoque', data_processamento=DATA_PROCESSAMENTO, coluna_date='data_referencia', coluna_id='produto_id', append_only=True)
        ingest_bronze_to_silver(sistema='sistema_vendas', dominio='vendas', nome_tabela='vendas', prefixo='vendas', data_processamento=DATA_PROCESSAMENTO, coluna_date='data_venda', coluna_id='pedido_id', append_only=True)
        ingest_bronze_to_silver(sistema='sistema_frete', dominio='frete', nome_tabela='frete', prefixo='frete', data_processamento=DATA_PROCESSAMENTO, coluna_date='data_envio', coluna_id='pedido_id')
        ingest_bronze_to_silver(sistema='sistema_tributos', dominio='tributos', nome_tabela='tributos', prefixo='tributos', data_processamento=DATA_PROCESSAMENTO, coluna_date='data_pagamento', coluna_id='pedido_id', append_only=True)